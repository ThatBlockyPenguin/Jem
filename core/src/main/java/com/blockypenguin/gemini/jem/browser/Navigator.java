...
public final class Navigator {
    ...
    public void root() {
        go(addresses.peek().map(CoreUtils::getURLRoot).orElseThrow());
    }

    public void up() {
        go(addresses.peek().map(CoreUtils::getURLUpper).orElseThrow());
    }
    ...
    public void reload() {
        BrowserManager.getUserInterface().setRendererComponent(null);
    ...
    public void go(URL originalUrl, boolean affectHistory, boolean affectNavBar, boolean getFromCache) {
        ...
        SwingUtilities.invokeLater(
            () -> {
                BrowserManager.getUserInterface().checkNavButtons(addresses.hasAt(-1), addresses.hasAt(1), url);
        ...
        BrowserManager.EXECUTOR.execute(ExceptionalRunnableFactory.create(
            () -> {
                if(affectNavBar) BrowserManager.getUserInterface().setNavURLText(url.toString());
            ...
            ex -> BrowserManager.showErrorDialogue(
                EXECUTOR_ERR_LOGGER,
                ex,
                "An error occurred whilst navigating to {}!",
                url
            )
        ));
    }
    ...
    private void renderDocument(Document doc) {
        ...
        BrowserManager.getRenderer(doc.mime().type()).ifPresent(
            iRenderer -> BrowserManager.getUserInterface().setRendererComponent(
